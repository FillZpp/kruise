...
import (
	...
	"github.com/openkruise/kruise/pkg/ctrlmesh/proxy/webhook/router"
	ctrlmeshutil "github.com/openkruise/kruise/pkg/ctrlmesh/util"
	"github.com/openkruise/kruise/pkg/util/bufferpool"
	"github.com/openkruise/kruise/pkg/webhook/util/certwatcher"
...
func (p *Proxy) Start(stop <-chan struct{}) (<-chan struct{}, error) {
	...
	go func() {
		...
		for {
			...
			err = ctrlmeshutil.CheckTCPPortOpened("127.0.0.1", p.opts.WebhookPort)
			if err == nil {
				break
			}
		...
		srv := &http.Server{
			Handler: &handler{
				router: router.New(p.opts.GrpcClient),
				cache:  cache,
			},
		}
...
func (h *handler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	routeResult, err := h.router.Route(ar.Request)
	if err != nil {
		writeResponse(rw, admission.Errored(http.StatusInternalServerError, err))
		return
	}

	var tripper http.RoundTripper
	switch {
	case routeResult.Ignore != nil:
		writeResponse(rw, admission.Allowed(""))
		return
	case routeResult.Accept != nil:
		tripper = h.cache.getTripper("127.0.0.1")
	case routeResult.Redirect != nil:
		tripper = h.cache.getOneTripper(routeResult.Redirect.Hosts)
	default:
		if routeResult.Reject != nil {
			writeResponse(rw, admission.Errored(routeResult.Reject.Code, routeResult.Reject.Error))
		}
		return
	}
	...
	p := ctrlmeshutil.NewSingleHostReverseProxy(u)
