...
var (
	...
	initialPods = []*v1.Pod{
		...
		{
			ObjectMeta: metav1.ObjectMeta{Name: "pod6", Namespace: "kube-system", Labels: map[string]string{"app": "bar"}},
			Spec:       v1.PodSpec{NodeName: "node5"},
		},
		{
	...
	initialJobs = []*appsv1alpha1.ImagePullJob{
		...
		},
		{
			...
			Spec: appsv1alpha1.ImagePullJobSpec{
				PodSelector: &appsv1alpha1.ImagePullJobPodSelector{AllNamespaces: true, LabelSelector: metav1.LabelSelector{MatchLabels: map[string]string{"app": "bar"}}},
			},
		},
	}
)
...
func testGetNodeImagesForJob(g *gomega.GomegaWithT) {
	...
	g.Expect(getNodeImagesForJob(initialJobs[6])).Should(gomega.Equal([]string{"node1", "node5"}))
}
...
func testGetActiveJobsForPod(g *gomega.GomegaWithT) {
	...
	g.Expect(getActiveJobsForPod(initialPods[1])).Should(gomega.Equal([]string{"job7"}))
	...
	g.Expect(getActiveJobsForPod(initialPods[5])).Should(gomega.Equal([]string{"job7"}))
...
func testGetActiveJobsForNodeImage(g *gomega.GomegaWithT) {
	...
	g.Expect(getActiveJobsForNodeImage(initialNodeImages[4])).Should(gomega.Equal([]string{"job1", "job3", "job7"}))
}
