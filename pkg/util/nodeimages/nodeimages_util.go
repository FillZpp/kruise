...
func GetNodeImagesForJob(reader client.Reader, job *appsv1alpha1.ImagePullJob) (nodeImages []*appsv1alpha1.NodeImage, err error) {
	...
	if job.Spec.PodSelector != nil {
		...
		podList := &v1.PodList{}
		if err := reader.List(context.TODO(), podList, client.InNamespace(getNamespaceForPodSelector(job)), client.MatchingLabelsSelector{Selector: selector}); err != nil {
			return nil, err
		}
...
func GetActiveJobsForPod(reader client.Reader, pod, oldPod *v1.Pod) (newJobs, oldJobs []*appsv1alpha1.ImagePullJob, err error) {
	...
	for i := range jobList.Items {
		...
		if job.Spec.PodSelector != nil {
			if !job.Spec.PodSelector.AllNamespaces && job.Namespace != pod.Namespace {
				continue
			}
...
func getNamespaceForPodSelector(job *appsv1alpha1.ImagePullJob) string {
	if job.Spec.PodSelector.AllNamespaces {
		return ""
	}
