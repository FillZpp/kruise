...
func validate(obj *appsv1alpha1.ImagePullJob) error {
	...
	if obj.Spec.PodSelector != nil {
		...
		if obj.Spec.PodSelector.AllNamespaces && len(obj.Spec.PodSelector.MatchLabels) == 0 && len(obj.Spec.PodSelector.MatchExpressions) == 0 {
			return fmt.Errorf("pod selector can not match all namespaces with no selector")
		}
	}
